...
export default function FaqsAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        setMessage({ type: 'success', text: 'FAQ uğurla yeniləndi!' });
      } else {
        await createFaq(formData);
        setMessage({ type: 'success', text: 'FAQ uğurla yaradıldı!' });
      }
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Bu FAQ-u silmək istədiyinizə əminsiniz?')) return;

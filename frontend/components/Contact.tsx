      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900">
            {language === 'en' ? 'Get in touch.' : 'Bizimlə əlaqə saxlayın.'}
          </h2>
        </motion.div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {data.map((contact, i) => (
            <motion.a
              ...
              target={['instagram', 'whatsapp'].includes(contact.type) ? '_blank' : undefined}
              rel={['instagram', 'whatsapp'].includes(contact.type) ? 'noopener noreferrer' : undefined}
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.5, delay: i * 0.08 }}
              className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md hover:border-gray-300 transition-all flex flex-col items-center text-center group"
            >
              <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center text-gray-600 mb-4 group-hover:bg-[#0A4D2C] group-hover:text-white transition-all">
